...
| 역사적 변동성 HV20·HV60 · BB폭 비율 | 120일 일봉 직접 계산 |
| ADX(14) · ATR(14) · OBV 다이버전스 · 피보나치 되돌림 | 120일 일봉 직접 계산 (레짐 인식 스코어링 + ATR 기반 손절) |
| 최신 뉴스 최대 6건 (발행일 포함) | Yahoo Finance |
| 실적 발표일·예상 EPS·서프라이즈 | Yahoo Finance |
| 실시간 현재가 (정규장·장전·장후) | Yahoo Finance / 한국투자증권 |

> 프롬프트에는 "오늘 날짜"와 "지표 기준일"(마지막 봉 날짜)을 함께 전달해, 두 날짜가 며칠 이상 차이 나면 AI가 그 공백을 리스크에 반드시 언급하도록 지시합니다. 현재가도 일봉 종가가 아니라 실시간 시세를 우선 사용하고, 실패 시에만 종가로 폴백합니다.
...
2순위: DB — 오늘의 추천 종목 분석 (생성 시 이미 Gemini로 분석된 결과 재사용, 재호출 없음)
3순위: 서버 인메모리 캐시 (10분 TTL)
4순위: Gemini 신규 호출
```

새로고침 버튼은 항상 Gemini를 새로 호출하고 캐시를 최신화합니다. 어느 목록(포지션 / 오늘의 추천)에서 종목 상세로 들어왔는지에 따라 1·2순위 우선순위가 바뀌며, 선호하는 쪽에 데이터가 없으면 자동으로 다른 쪽으로 폴백합니다.

> AI 미연결 시 → RSI(±2) + 볼린저밴드(±2) + MACD(±1) + MA크로스(±1) + 거래량(±1) 점수제 폴백 엔진 자동 작동

### 📰 오늘의 추천 종목 (`/recommendations`)

매일 자동으로 섹션별 강세 섹터를 뽑고, 그 섹터에 맞는 미국·한국 종목을 Gemini가 골라 전략까지 미리 분석해두는 기능.

- 9개 섹터 ETF 1개월 수익률 상위 3개 섹터 선정 → 섹터별로 미국 10종목 + 한국 10종목을 Gemini가 선정 (US/KR 분리 호출로 JSON 잘림 방지)
- 선정된 각 종목을 실제로 분석해 `strategy`·`snapshot`까지 DB에 저장 — 종목 상세 페이지 진입 시 이 결과를 그대로 재사용 (Gemini 재호출 없음)
- **섹터 필터 토글** — 시그널 필터(전체/강력매수/매수/관망)와 같은 스타일로 섹터별로도 나눠볼 수 있음
- 7일 초과된 과거 추천 데이터는 다음 생성 실행 시 자동 삭제
- **자동 생성**: GitHub Actions가 매일 KST 06:00~07:55 사이 5분 간격으로 `/api/recommendations/generate`를 호출 (`.github/workflows/recommendations-generate.yml`). 1회 호출은 52초 예산 안에서 처리 가능한 만큼만 처리하고, 이미 처리된 종목은 건너뛰며 다음 호출에서 이어서 처리
- 관리자 계정은 페이지에서 "이어서 생성" 버튼으로 수동 실행 가능

> GitHub Actions의 scheduled 트리거는 "최선을 다해" 실행되는 방식이라 빈도가 높으면(5분 간격) 부하 상황에 스킵될 수 있습니다 — 시간에 민감하면 수동 실행으로 보정이 필요할 수 있습니다.

### 📊 종목 상세 분석

- 120일 캔들 차트 + RSI·MACD·볼린저밴드·이동평균선·거래량 오버레이 + ADX·ATR·OBV·피보나치 탭
...
| 구분 | 출처 | 판별 방식 |
|------|------|----------|
| 미국 장전·장후 | Yahoo Finance 차트 API (`interval=1m`, `includePrePost=true`) | 차트 메타의 `currentTradingPeriod`(pre/regular/post 시작·종료 시각)를 직접 계산해 현재 세션 판별, 해당 구간의 마지막 1분봉 종가를 시간외가로 사용 |
...
>
> 미국 쪽은 원래 Yahoo v7 quote API(`marketState`)로 판별했으나, 이 엔드포인트가 401 Unauthorized로 막혀 v8 차트 API 기반 방식으로 교체했습니다.
...
DailyRecommendation  — 일별 섹터 기반 종목 추천 (전략·스냅샷 포함, date+ticker 유니크)
KisTokenCache        — 한국투자증권 API 토큰 캐시 (Vercel 인스턴스 간 공유)
```
...
├── app/
│   ├── api/
│   │   ├── strategy/             — AI 전략 분석 (포지션/추천 재사용 + Gemini 신규 호출)
│   │   ├── recommendations/      — 오늘의 추천 조회 + generate/ (섹터 선정·분석·DB 저장)
│   │   ├── positions, watchlist, rankings, market, stock, prices, quotes, search 등
│   │   └── admin/                — 접근 요청·사용자 관리
│   ├── admin/          — 관리자 페이지
│   ├── positions/      — 포지션 관리 페이지
│   ├── recommendations/ — 오늘의 추천 종목 페이지 (섹터·시그널 필터)
...
│   ├── IndicatorExplainer.tsx  — 지표 해설 카드 (RSI·MACD·BB·ADX·ATR·OBV·피보나치)
│   ├── PriceSpectrumBar.tsx    — 가격 스펙트럼 바 시각화
│   ├── SkeletonBox.tsx         — Shimmer 로딩 스켈레톤
│   ├── StockChart.tsx          — 캔들 차트 + 지표 오버레이 (ADX/OBV 탭 포함)
│   ├── SearchBar.tsx           — 종목 검색
│   ├── AuthGuard.tsx           — 비로그인 사용자 리다이렉트 래퍼
│   ├── BottomNav.tsx           — 모바일 하단 탭 내비게이션
│   └── Header.tsx, SessionProvider.tsx, TickerIcon.tsx
├── hooks/
...
├── lib/
│   ├── indicators.ts      — 기술적 지표 계산 (RSI·MACD·BB·MA·HV·BBW·ADX·ATR·OBV·피보나치)
│   ├── dataSource.ts      — 데이터소스 통합 (Yahoo Finance / 한국투자증권)
│   ├── livePrice.ts       — 실시간 현재가 조회 (정규장·장전·장후, US/KR 공통)
│   ├── strategyAnalyzer.ts — Gemini 프롬프트 빌더 + 전략 분석 오케스트레이션 (전략/추천 라우트 공유)
│   ├── kisToken.ts        — 한국투자증권 API 토큰 발급·캐싱
│   ├── prisma.ts          — Prisma Client 싱글톤
│   └── auth.ts            — NextAuth 설정
└── types/
    ├── stock.ts        — OHLCVBar, StrategyResult, Position, IndicatorSnapshot 등
...
| 미국 주가·차트·시간외·실시간가 | Yahoo Finance (비공식 차트/검색 API) |
| 한국 주가·차트·시간외·실시간가 | 한국투자증권 Open API |
| 시장·뉴스·실적 | Yahoo Finance |
| 데이터베이스 | Supabase PostgreSQL (Prisma v7) |
| 캐시 | `globalThis` 인메모리 · TTL 관리 |
| 배포 | Vercel |
| DB 활성화 유지 | Vercel Cron (매일 자정) |
| 추천 종목 자동 생성 | GitHub Actions (매일 KST 06:00~07:55, 5분 간격) |
...
| `NEXT_PUBLIC_ADMIN_EMAIL` | 클라이언트에서 어드민 여부 판별용 (`ADMIN_EMAIL`과 동일 값) — 추천 페이지 "이어서 생성" 버튼 노출에 사용 |
| `CRON_SECRET` | `/api/recommendations/generate`를 호출하는 GitHub Actions 인증용 토큰 (아래 "추천 종목 자동 생성" 참고) |
| `DISCORD_WEBHOOK_URL` | 포지션 등록·종료 알림 웹훅 (선택) |
| `DISCORD_ACCESS_REQUEST_WEBHOOK_URL` | 접근 요청 알림 웹훅 (선택) |

> `GEMINI_API_KEY`가 없어도 규칙 기반 폴백 엔진으로 모든 기능이 작동합니다. 미국 주가·차트는 별도 API 키 없이 Yahoo Finance를 사용합니다.
...
4. Push 시 자동 배포
5. Vercel Cron은 `vercel.json`에 정의되어 있어 별도 설정 불필요 (`/api/health` DB 활성화 유지용)
6. **추천 종목 자동 생성을 쓰려면** GitHub 저장소 **Settings → Secrets and variables → Actions**에 `APP_URL`(배포된 서비스 URL)과 `CRON_SECRET`(`.env.local`과 동일 값)을 등록 — `.github/workflows/recommendations-generate.yml`이 이 값으로 매일 자동 실행됩니다

---

## 배운 것들

- **외부 API 조합**: Yahoo Finance(비공식), 한국투자증권 API를 하나의 스냅샷으로 합치는 설계
- **비공식 API의 무상통보 변경 대응**: Yahoo v7 quote API가 어느 날 401로 막혀 시간외가 조회가 전부 조용히 실패하던 문제 — 여전히 열려있는 v8 차트 API(1분봉 + includePrePost)로 세션 판별 로직을 직접 재구현해 대체
- **AI 프롬프트 엔지니어링**: 정형화된 JSON 전략을 안정적으로 반환하게 Gemini 프롬프트 설계, "오늘 날짜"·"지표 기준일"을 명시해 데이터 시차를 AI가 스스로 판단하게 유도
- **폴백 패턴**: AI 장애 상황을 상정하고 규칙 기반 엔진을 병렬 유지하는 방식
- **캐시 전략**: DB(포지션 → 추천) → 인메모리 캐시 → API 호출 순으로 우선순위를 두어 외부 API 쿼터 보호, 진입 경로(포지션/추천 목록)에 따라 우선순위를 동적으로 조정
