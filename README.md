...
주식 공부를 하면서 "RSI가 30이면 매수해야 하나?", "이 뉴스가 호재야 악재야?" 같은 판단을 매번 직접 하기가 번거로워서 만들었습니다.

기술적 지표(RSI, MACD, 볼린저 밴드 등) + 최신 뉴스 + 실적 발표 데이터를 모아서 Google Gemini AI한테 분석을 맡기고, 결과로 구체적인 매수·매도 전략(진입가, 손절가, 목표가)을 받아볼 수 있습니다. AI 연결이 안 될 때는 5개 지표 점수제 규칙 기반 엔진이 자동으로 대신합니다.
...
| 데이터 | 출처 |
|--------|------|
| 기술적 지표 (RSI·MACD·BB·MA) | 120일 일봉 직접 계산 |
...
손절 전략: 손절가 + 기술적 근거 (MA·BB 레벨 명시)
목표 전략: 1차·2차 목표가 + 기술적 근거
리스크: 실적 발표 일정·섹터 리스크 등
```

> AI 미연결 시 → RSI(±2) + BB(±2) + MACD(±1) + MA크로스(±1) + 거래량(±1) 점수제 폴백 엔진 자동 작동

### 📊 종목 상세 분석
- 120일 캔들 차트 + RSI·MACD·볼린저밴드·이동평균선 오버레이
- 한국·미국 종목 모두 지원
